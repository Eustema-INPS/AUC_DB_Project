...
-- Aggiornamento: 2015.11.25 Ottimizzazione fatta da Francesco Bonetti
-- Modificata da: Raffaele
-- Data:		2016.10.03
-- Descrizione:	Conversione getdate
-- =============================================
CREATE PROCEDURE  [dbo].[SP_FINDSOGGCONTR_POSCONTR_BYSOGGCOLL]
		...
		BEGIN
			...
			Insert into @TT_AURSS
				...
				FROM tb_ausco_sog_contr_col INNER JOIN tb_aurss_rel_sog_sog ON 
					 tb_ausco_sog_contr_col.ausco_codice_pk = tb_aurss_rel_sog_sog.aurss_ausco_codice_pk INNER JOIN tb_autis_tipo_sog_col_ct ON tb_aurss_rel_sog_sog.aurss_autis_codice_pk = tb_autis_tipo_sog_col_ct.autis_codice_pk INNER JOIN tb_ausca_sog_contr_az ON tb_aurss_rel_sog_sog.aurss_ausca_codice_pk = tb_ausca_sog_contr_az.ausca_codice_pk
				WHERE tb_ausco_sog_contr_col.ausco_codice_fiscale = @codice_fiscale
				AND ( tb_aurss_rel_sog_sog.aurss_autis_codice_pk = 89 
				      OR tb_aurss_rel_sog_sog.aurss_autis_codice_pk = 90
				...
				(
                 ...
                 SELECT   
					  ...
					  INTO #TEMP01
					  FROM tb_ausco_sog_contr_col INNER JOIN tb_aursp_rel_sog_pos ON 
						 tb_ausco_sog_contr_col.ausco_codice_pk = tb_aursp_rel_sog_pos.aursp_ausco_codice_pk INNER JOIN tb_autis_tipo_sog_col_ct ON 
						 tb_aursp_rel_sog_pos.aursp_autis_codice_pk = tb_autis_tipo_sog_col_ct.autis_codice_pk INNER JOIN tb_aupoc_pos_contr ON 
						 tb_aursp_rel_sog_pos.aursp_aupoc_codice_pk = tb_aupoc_pos_contr.aupoc_codice_pk INNER JOIN tb_ausca_sog_contr_az ON 
						 tb_aupoc_pos_contr.aupoc_ausca_codice_pk = tb_ausca_sog_contr_az.ausca_codice_pk INNER JOIN tb_aurea_area_gestione ON 
						 tb_aupoc_pos_contr.aupoc_aurea_codice_pk = tb_aurea_area_gestione.aurea_codice_pk INNER JOIN tb_auspc_stato_pos_contr_ct ON 
					  ...
					  WHERE     
                       ...
                       ausca_codice_fiscale  not in 
							(SELECT B.[Codice_Fiscale]  FROM @TT_AURSS B ) 
							AND	(aursp_autis_codice_pk = 97) 
							AND (aursp_data_di_fine_validita >= convert(date,GETDATE(),103)) 
							AND (aursp_data_inizio_validita <= convert(date,GETDATE(),103)) 
							OR	(aursp_autis_codice_pk = 97) 
							AND (aursp_data_di_fine_validita IS NULL) 
							OR	(aursp_autis_codice_pk = 97) 
							AND (aursp_data_inizio_validita IS NULL)
