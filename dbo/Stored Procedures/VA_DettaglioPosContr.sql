...
CREATE PROCEDURE [dbo].[VA_DettaglioPosContr]
...
--FINE modifica Quirino 24/02/2014
		...
		(SELECT COUNT(tb_aurpp_rel_poc_poc.aurpp_codice_pk)
		 ...
		 WHERE tb_aurpp_rel_poc_poc.aurpp_aupoc_cod_pk_figlia=aupoc_codice_pk) 
		 AS DB_NumPosizioniSecondarie,
		 
		(SELECT COUNT(tb_aurpp_rel_poc_poc.aurpp_codice_pk) 
		 FROM tb_aurpp_rel_poc_poc
		 WHERE tb_aurpp_rel_poc_poc.aurpp_aupoc_cod_pk_madre=aupoc_codice_pk)   
		...
		--AI 3115
		,convert(varchar(10),aupoc_codice_pk) + convert(varchar(2),aupoc_contro_codice)  as DB_Chiave_AUC
		,aupoc_codice_pk as DB_Chiave_Aupoc
		,aupoc_data_modifica as DB_Data_Modifica_Aupoc
	...
	LEFT OUTER JOIN tb_auind_indirizzi ON
		UPPER(auind_tabella) = 'AUPOC' AND auind_tabella_codice_pk = aupoc_codice_pk
	...
	on [dbo].[tb_aurpp_rel_poc_poc].aurpp_codice_pk = aupoc_codice_pk
