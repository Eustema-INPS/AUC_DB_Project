...
-- Per il Modello 046036
-- ===============================================================================================
CREATE PROCEDURE [dbo].[SP_MOD_046036_IN_AURAZ] 
	@DtInizio date, 
	@DtFine date
	AS
BEGIN
	...
	BEGIN
		...
		INSERT INTO [dbo].[tb_auraz_report_azst] 
			(auraz_codice_modello,
			 ...
			 Auraz_data_inizio,
			 Auraz_data_fine)
			SELECT @codiceModello, 0, autaz_azstapro, autaz_azstazon --'046036'
			  , 0, 0, 0  				
			  , SUM (CASE WHEN autaz_azcodopr = '09' AND autaz_azproven IN (2, 4) AND autaz_azflagop = 1 
					THEN 1 ELSE 0 END) DEFA          
			  , SUM (CASE WHEN autaz_azcodopr = '09' AND autaz_azproven IN (0, 1)                            
					THEN 1 ELSE 0 END) DEFR1                   
			  , SUM (CASE WHEN autaz_azcodopr = '09' AND autaz_azproven IN (2, 4) AND autaz_azflagop = 2  
		...
		WHERE auraz_codice_modello = '046036' and auraz_stato = 0
